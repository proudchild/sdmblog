...
if(strlen(get_the_title($post->ID))<23)
	...
	$html .= '
					<a class="post-title" href="'.get_permalink($post->ID).'">'.substr(get_the_title($post->ID), 0 , 20).'...</a>'.($args['show_post_views'] ? ' <span class="count">('.number_format_i18n(pvc_get_post_views($post->ID)).')</span>' : '');
